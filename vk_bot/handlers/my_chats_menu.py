...
from vk_bot.template_messages import BACK_SERVICES_MENU, COMMAND_NOT_FOUND, NOT_ENOUGH_COINS, \
    REPEAT_CHAT_SUBSCRIPTION
from vk_bot.keyboards.other import repeat_chat_subscription_keyboard
from vk_bot.keyboards.services_menu import get_services_menu_keyboard
from vk_bot.keyboards.my_chats_menu import get_my_chats_keyboard, get_management_chat_keyboard, \
...
async def handler_my_chats_menu(
    ...
    if extra_data.menu == MyChatsMenu.CHATS and chat_id is None:
        ...
        if message == "назад":
            ...
            if chat_data.owner_id == owner_id:
        ...
        if message == "назад":
            ...
            chat_name = chat_data.name if chat_data.name else int(chat_id - 2E9)
        ...
        if message == "назад":
            ...
            chat_cost = CHAT_TYPE_COST[chat_data.type]
        ...
        if confirm is True:
            ...
            if owner_data.coins < extra_data.prolong_cost:
                ...
                chat_name = chat_data.name if chat_data.name else int(chat_data.chat_id - 2E9)
                days_word = get_word_case(prolong_period, ("день", "дня", "дней"))

                await NotificationsService.send_notification(
                    chat=NotifyChats.MAIN,
                    message=f"{owner_data.vk_name} продлил беседу {chat_id} со статусом " \
                        f"{chat_data.type.value} на {prolong_period}д за {format_number(prolong_cost)}"
                )
