...
from vk_bot.keyboards.clans_menu import get_create_clan_keyboard, \
...
async def handler_menu_create_clan(
    ...
    if extra_data.menu == CreateClanMenu.MAIN:
        ...
        if message == "назад":
            ...
            await ClanService.send_keyboard_clan_menu(user_data, psql_cursor)
        ...
        if message == "назад":
            ...
            clan_name = UserSchema.format_vk_name(user_id, clan_name)
            await NotificationsService.send_notification(
                chat=NotifyChats.CREATE_CLAN,
                message=f"🏰 {user_data.vk_name} создал клан {clan_name}"
            )
