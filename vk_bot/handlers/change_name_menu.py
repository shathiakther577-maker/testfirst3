...
from vk_bot.template_messages import BACK_MAIN_MENU, PATTERN_BANNED_SYMBOLS, \
    NOT_ENOUGH_COINS
...
async def handler_change_name_menu(
    ...
    if message == "назад":
        ...
        old_user_name = UserSchema.format_vk_name(user_id, user_data.full_name)
        new_user_name = UserSchema.format_vk_name(
        ...
        await NotificationsService.send_notification(
            ...
            message=f"✍ {old_user_name} сменил имя на {new_user_name}"
        )

        response = f"✅ Ваш ник изменен на {new_user_name}"
        keyboard = get_main_menu_keyboard(user_data)
        update_user_menu(user_id, UserMenu.MAIN, psql_cursor)
