...
from modules.vkontakte.chats import get_chat_owner_id
from modules.vkontakte.bot import send_message

from vk_bot.template_messages import NOT_ENOUGH_COINS
from vk_bot.keyboards.other import empty_keyboard
from vk_bot.keyboards.unactive_chat import get_keyboard_select_game_mode, \
...
async def handler_inactive_chat(
    ...
    if chat_data is None:
        ...
        response = """
            ...
            Для игры здесь выбери режим (одна из кнопок ниже) и оплати беседу ИЛИ зайди в одну из официальных бесед - напиши боту в ЛС и нажми кнопку "Найти беседу".
    ...
    if (
        ...
        if user_data.coins >= CHAT_TYPE_COST[chat_type] or user_data.status == UserStatus.ADMIN:
            ...
            await NotificationsService.send_notification(
                ...
                message=f"{user_data.vk_name} активировал беседу {chat_id} со статусом {chat_type.value}"
            )
