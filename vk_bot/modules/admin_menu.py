...
def get_time_stamp_keyboard(*, event: str) -> str:
    ...
    keyboard.add_button(
        ...
        payload={"event": event, "time_stamp": TimeStamp.DAY.value}
    )
    keyboard.add_button(
        label="Неделя",
        color=VkKeyboardColor.POSITIVE,
        payload={"event": event, "time_stamp": TimeStamp.WEEK.value}
    )
    keyboard.add_line()
    ...
    keyboard.add_button(
        label="Все время",
        color=VkKeyboardColor.POSITIVE,
        payload={"event": event, "time_stamp": TimeStamp.ALL.value}
    )
...
class AdminPanel:
    ...
    async def get_user_data(
        ...
        if user_data is None:
            raise UserDataNotFound(f"❌ {UserSchema.format_vk_name(user_id, 'Пользователь')} не зарегистрирован")
    ...
    def get_chat_data(
        ...
        chat = int(chat)
        chat_id = chat if chat >= 2E9 else chat + 2E9
