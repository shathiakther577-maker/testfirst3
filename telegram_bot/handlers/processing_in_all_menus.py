...
from telegram_bot.template_messages import CLAN_GREETING, USER_NOT_FOUND, CLAN_NOT_FOUND, \
    YOU_HAVE_CLAN, USER_HAVE_CLAN, MAX_COUNT_MEMBERS_IN_CLAN, APPLICATION_EXPIRED, \
    DATA_OUTDATED
from telegram_bot.keyboards.main_menu import get_game_selection_keyboard
from telegram_bot.keyboards.clans_menu import get_clan_member_keyboard
...
async def processing_in_all_menus(
    ...
    if (
        ...
        if user_data is None:
            ...
            await ClanService.send_clan_owner_notification(
                clan_data=clan_data,
                message=f"⚠️ {user_data.full_name} принял приглашение в клан",
                mandatory=True
            )
            ...
            await ClanService.send_clan_owner_notification(
                clan_data=clan_data,
                message=f"⚠️ {user_data.full_name} отказался от приглашения в клан",
                mandatory=True
            )
    ...
    if (
        ...
        if user_data is None:
            ...
            clan_name = UserSchema.format_telegram_name(clan_data.owner_id, clan_data.name)
            await send_message(
                chat_id=user_id,
